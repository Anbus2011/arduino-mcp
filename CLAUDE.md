...
**VERSION: 1.0.0** (keep in sync with package.json; bump per change)
...
- Server exports its internals (`runCli`, `writeSketch`, `parseCompileResult`, …) via `module.exports` and only starts the stdio transport when `require.main === module` — `scripts/smoke.js` exercises the exact same code paths.
...
- `node scripts\smoke.js` — **the smoke test**: compiles a known-good Basicmicro sketch for `leonardo` (expects ok=true with flash/RAM figures) and the same sketch minus the include (expects ok=false with a line-numbered error diagnostic). Exits non-zero on any mismatch. Run after any server.js change.
